...
# Visualize the confusion matrix with a purple color scheme
plt.figure(figsize=(12, 10))
sns.heatmap(cm_percentage, annot=True, fmt='.2f', cmap='Purples', xticklabels=label_names, yticklabels=label_names, cbar_kws={'label': 'Percentage'}, annot_kws={"size": 16})
...
importance_dict = xgboost_model.get_score(importance_type='weight')  # Feature importance based on weight
...
# Feature importance visualization with a purple color scheme
plt.figure(figsize=(12, 8))
sns.barplot(x='Normalized Importance', y='Feature', data=importance_df, color='purple')
plt.title('Feature Importance (XGBoost)', fontsize=22)
