...
# 데이터 전처리
data = data[['label', 'blood_uric_acid']].dropna()  # 'blood_uric_acid'로 변경
...
# 특징과 레이블 분리
features = data[['blood_uric_acid']]  # 'blood_uric_acid'로 변경
labels = data['label']

# StandardScaler로 피처 표준화
scaler = StandardScaler()
features = pd.DataFrame(scaler.fit_transform(features), columns=['blood_uric_acid'])
...
# Confusion Matrix 시각화 (빨간색)
plt.figure(figsize=(10, 8))
sns.heatmap(
    cm_percentage, annot=True, fmt='.2f', cmap='Reds',  # Changed to 'Reds' colormap
    xticklabels=label_names, yticklabels=label_names,
    cbar_kws={'label': 'Percentage'}, annot_kws={"size": 14}
)
plt.title('Confusion Matrix (XGBoost) - Blood Uric Acid Classification', fontsize=20)
